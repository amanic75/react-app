-- App statistics script - works with JSONB app_access field

-- 1. First, let's see what tables we have
SELECT table_name 
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('apps', 'company_apps', 'formulas', 'raw_materials', 'suppliers')
ORDER BY table_name;
...
-- 3. Create function to get app statistics (fixed for JSONB app_access)
...
BEGIN
  ...
  -- Fixed to work with JSONB app_access field
  ...
  AND (
    -- Check if app_type is in the JSONB app_access array
    up.app_access ? p_app_type
