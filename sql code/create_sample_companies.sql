-- ==============================================
-- CREATE SAMPLE COMPANIES FOR TESTING DATA CONSISTENCY
...
-- Insert corresponding company apps for each company
INSERT INTO company_apps (company_id, app_id, app_name, enabled, configuration, deployed_at) 
SELECT 
    c.id as company_id,
    unnest(c.initial_apps) as app_id,
    CASE 
        WHEN unnest(c.initial_apps) = 'formulas' THEN 'Formulas'
        WHEN unnest(c.initial_apps) = 'raw-materials' THEN 'Raw Materials'
        WHEN unnest(c.initial_apps) = 'suppliers' THEN 'Suppliers'
        ELSE 'Unknown'
    END as app_name,
    true as enabled,
...
BEGIN
    ...
    RAISE NOTICE '✅ Sample companies created successfully!';
    RAISE NOTICE '';
    RAISE NOTICE 'Summary:';
    RAISE NOTICE '- Total companies: %', company_count;
    RAISE NOTICE '- Total company apps: %', app_count;
    RAISE NOTICE '';
    RAISE NOTICE 'Companies created:';
    RAISE NOTICE '1. Capacity Chemicals (admin@capacity-chemicals.com)';
    RAISE NOTICE '2. ChemTech Solutions (sarah.admin@chemtech-solutions.com)';
    RAISE NOTICE '3. Industrial Formulators Inc (david.admin@industrial-formulators.com)';
    RAISE NOTICE '';
    RAISE NOTICE 'Next steps:';
    RAISE NOTICE '1. Go to NSight Admin Dashboard → Existing Company Mode';
    RAISE NOTICE '2. Click "Sync Companies" to create admin user accounts';
    RAISE NOTICE '3. Test that admin accounts can log in and access their company data';
END $$;

-- Show the companies that were created
SELECT 
    company_name,
    admin_user_name,
    admin_user_email,
...
-- Show the apps that were created for each company
SELECT 
    c.company_name,
    ca.app_name,
    ca.enabled,
