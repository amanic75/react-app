...
/**
 * Fetch all materials from the backend with company filtering to prevent data flashing.
 * @param {Object} options - Options including userProfile for filtering
 * @returns {Promise<{data: Array, error: any}>}
 */
export const getAllMaterials = async (options = {}) => {
  try {
    ...
    if (options.userProfile) {
      if (options.userProfile.role === 'NSight Admin') {
        // NSight Admins see all materials
        query = query.order('id', { ascending: true });
      } else if (options.userProfile.company_id) {
        // Other users only see their company's materials
        query = query
